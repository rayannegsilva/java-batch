...
public class BatchConfig {
  ...
  Step step(ItemReader<TransactionCnab> reader,
            ItemProcessor<TransactionCnab, Transacao> processor,
            ItemWriter<Transacao> writer) {
    return new StepBuilder("step", jobRepository)
        .<TransactionCnab, Transacao>chunk(1000, transactionManager)
        .reader(reader)
        .processor(processor)
        .writer(writer)
        .build();
  ...
  ItemProcessor<TransactionCnab, Transacao> processor() {
    return item -> {
      var transacao = new Transacao(
          null, item.tipo(), null, item.valor(), item.cpf(), item.cartao(),
